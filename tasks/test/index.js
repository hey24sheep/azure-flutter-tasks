...
function runTests(flutter, concurrency, updateGoldens, name, plainName, canGenerateCodeCoverage, extraArgs) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        testRunner.on('stdout', line => {
            ...
            const loadingTestRegex = /\s*\d\d:\d\d (\+\d+)?(\s+\-\d+)?:\s*loading\s*(.*\.dart)\s*/;
            let loadingMatch = loadingTestRegex.exec(line);
            if (loadingMatch && loadingMatch[3]) {
                basename = loadingMatch[3];
            }
            else {
                const testSuiteRegex = /\s*\d\d:\d\d (\+\d+)?(\s+\-\d+)?:\s*(?<title>.*\.dart):\s*\(setUpAll\)/;
                const setUpMatch = testSuiteRegex.exec(line);
                if (setUpMatch && setUpMatch.groups && setUpMatch.groups.title) {
                    basename = setUpMatch.groups.title;
                }
            }
            var newSuite = {
                title: basename ? path.basename(basename, ".dart") : 'AUTOGENERATED:ALL:TESTS',
                isSuccess: false,
                succeeded: 0,
                failed: 0,
                cases: []
            ...
            currentSuite = createTestCase(currentSuite, line);
